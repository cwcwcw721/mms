	...
	protected SwrveMessage _getMessageForEvent(String event, SwrveOrientation orientation) {
		...
		if (campaigns != null) {
			...
			if (!event.equalsIgnoreCase(SWRVE_AUTOSHOW_AT_SESSION_START_TRIGGER) && isTooSoonToShowMessageAfterLaunch(now)) {
				noMessagesWereShown(event, "{Game throttle limit} Too soon after launch. Wait until " + timestampFormat.format(showMessagesAfterLaunch));
				return null;
			}

			if (isTooSoonToShowMessageAfterDelay(now)) {
				noMessagesWereShown(event, "{Game throttle limit} Too soon after last message. Wait until " + timestampFormat.format(showMessagesAfterDelay));
				return null;
			}

			if (hasShowTooManyMessagesAlready()) {
				noMessagesWereShown(event, "{Game Throttle limit} Too many messages shown");
				return null;
			}
